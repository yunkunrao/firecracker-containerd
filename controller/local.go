...
/*
	Copy pasted from "firecracker-control".
	Firecracker controller functionality moves to a sandbox proxy plugin.
*/
...
func (s *local) CreateVM(requestCtx context.Context, req *proto.CreateVMRequest) (*proto.CreateVMResponse, error) {
	...
	if isEADDRINUSE(err) {
		return nil, status.Errorf(codes.AlreadyExists, "VM with ID %q already exists: %s", id, shimSocketAddress)
	} else if err != nil {
